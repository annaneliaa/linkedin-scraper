...
def get_conversations_details_by_profile(api, profile_id):
    ...
    details
    """
    profile_urn = crop_urn(api.get_profile(profile_id)["entityUrn"])
    ...
    conversation_details = {
        ...
        'last_activitity_at': convert_unix_time(result["lastActivityAt"]),
...
def get_unread_conversations(api):
    ...
    for i in range(len(conversations)):
        ...
        if(conv["unreadCount"] > 0):
            ...
            profile_urn = crop_urn(conv["participants"][0]["com.linkedin.voyager.messaging.MessagingMember"]["miniProfile"]["entityUrn"])

            # retrieve URN id of sender of last message
            sender = crop_urn(conv["events"][0]["from"]["com.linkedin.voyager.messaging.MessagingMember"]["miniProfile"]["entityUrn"])
            ...
            if(profile_urn == sender):
                ...
                # append to list
                d = {
                    "conversation_urn": crop_urn(conv_urn),
                    "profile_urn": crop_urn(profile_urn)
                }
                unread_conversations.append(d)
            else: continue
...
def get_days_since_last_message(api, self_id, profile_id):
    ...
    self_urn = get_urn_from_profile_id(api, self_id)
    ...
    sender = crop_urn(last_message["sender_urn"])
